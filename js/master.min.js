...
a=[];for(e=1;e<=d;e++)a.push(e);return a},this);a.removePair=function(b){a.rows.remove(b);a.rows.remove(b.matchingRow)};a.rows.subscribe(function(b){var e=new Currency("0");b.forEach(function(a){a.update(e)});a.loading||a.storage.setDates(b);var d=a.rows.indexOf(a.todayRow);if(-1!==d&&d<a.rows().length-1){a.nextRow=a.rows()[d+1];var d=util.stripTime(a.nextRow.date),c=util.stripTime(a.todayRow.date),d=(d-c)/864E5,c=Math.floor(d);if(c!==d)var f=Math.ceil(d),d=f-d<d-c?f:c;a.daysLeft(d)}else a.daysLeft(0);
b.length%2&&a.updateChart&&a.updateChart()});a.updateDuration=function(){var b=[];a.rows().forEach(function(a){a.isStart&&b.push({date:a.date,amount:a.amount})});a.deleteAll();b.forEach(function(b){a.addDate(b.date,b.amount)})};a.timeAmount.subscribe(a.updateDuration);a.timeUnit.subscribe(a.updateDuration);a.deleteAll=function(){a.rows.remove(function(a){return!a.isToday})};a.getInput=function(){var b=new Date(a.date()),c=new Currency(a.amount()),d=!0;c.valid||(d=!1,a.amountFocus(!0));a.amountError(!c.valid);
util.isInvalidDate(b)?(d=!1,a.dateError(!0),a.dateFocus(!0)):a.dateError(!1);d&&(a.date(""),a.amount(""),a.addDate(b,c),a.dateFocus(!0));return!1};a.timeAmount.subscribe(function(b){a.storage.setTimeAmount(b)});a.timeUnit.subscribe(function(b){a.storage.setTimeUnit(b)});a.rowMouseOver=function(b){a.unHighlightAllRows();a.highlight(b,!0)&&a.highlightPoints(b)};a.mouseOut=function(){a.unHighlightAllRows();a.highlightPoints()};a.nextMouseOver=function(){a.unHighlightAllRows();a.highlight(a.nextRow,!0)&&
a.highlightPoints(a.nextRow)};a.highlight=function(a){return a.isToday||a.isSizeRow?!1:(a.isHighlighted(!0).matchingRow.isHighlighted(!0),!0)};a.unHighlightAllRows=function(){a.rows().forEach(function(a){a.isHighlighted(!1)})}}
PageViewModel.prototype.addDate=function(c,f){var a=new Date(c),b=Number(this.timeAmount());switch(this.timeUnit()){case "Days":a.setDate(a.getDate()+b);break;case "Months":a.setMonth(a.getMonth()+b);break;default:console.log("Failed to find time unit, defaulting to years");case "Years":a.setFullYear(a.getFullYear()+b)}b=new RowViewModel(c,f,!0,this.isSmall);a=new RowViewModel(a,new Currency(f),!1,this.isSmall);b.matchingRow=a;a.matchingRow=b;this.rows.push(b);this.rows.push(a);this.rows.sort(function(a,
...
PageViewModel.prototype.loadSavedData=function(){this.loading=!0;this.storage.getDates().forEach(function(c){this.addDate(new Date(c[1]),new Currency(c[0]))},this);this.loading=!1};PageViewModel.prototype.getStartRows=function(){return this.rows().filter(function(c){return c.isStart})};
...
rc.selectPointHandler=function(){rc.hoverPointHandler(rc.lastPoint)};rc.highlightPoints=function(a){var c=rc.viewModel.todayRow,b=rc.viewModel.rows.indexOf(c);if("undefined"===typeof a||null===a)rc.setChartSelection([{row:b,column:1}]);else{var f=a.isStart?a:a.matchingRow,d=f.matchingRow,e=rc.viewModel.rows.indexOf(f),g=rc.viewModel.rows.indexOf(d);rc.setChartSelection([{row:e,column:1},{row:g,column:1},{row:b,column:1}])}rc.colorPoints(f,d,c,e,g,b)};
rc.colorPoints=function(a,c,b,f,d,e){rc.TODAY_COLOR||(rc.TODAY_COLOR=rc.$table.find(".active").children("td").first().css("backgroundColor"));a=$("svg>g>g>g>circle");c=[];for(b=0;b<a.length;b++)c.push(a[b].outerHTML);var g=1,k=2,h=3;c=4;b=5;6>a.length?rc.setSelectedPointColor(a[0],a[a.length-1],rc.TODAY_COLOR):(6<a.length&&("none"==a[2].getAttribute("fill")?(g+=3,k+=3,h+=3,c+=3,b+=3):"none"==a[4].getAttribute("fill")?(h+=3,c+=3,b+=3):"none"==a[6].getAttribute("fill")&&(b+=3)),e<f?(d=0,f=k,e=h,k=c,
h=b):e<d?(f=0,e=g,d=k,g=h,k=c,h=b):(f=0,e=g,d=c,g=b),rc.START_COLOR||(rc.START_COLOR=rc.$table.find("tr.start.highlight").first().css("backgroundColor")),rc.STOP_COLOR||(rc.STOP_COLOR=rc.$table.find("tr.stop.highlight").first().css("backgroundColor")),rc.setSelectedPointColor(a[f],a[e],rc.START_COLOR),rc.setSelectedPointColor(a[k],a[h],rc.STOP_COLOR),rc.setSelectedPointColor(a[d],a[g],rc.TODAY_COLOR))};
...
null,b=null,f=[["Date","Owed"]],d=0;d<a.length;d++)if(a[d].isToday)b=a[d];else{if(b){if(c)var e=a[d],g=c.date.getTime(),k=b.date.getTime(),h=e.date.getTime()-g,g=(k-g)/h,c=c.owed().toFloat(),e=e.owed().toFloat(),e=c+g*(e-c);else e=0;f.push(rc.buildDot(b,e));b=null}c=a[d];f.push(rc.buildDot(c))}b&&f.push(rc.buildDot(b));rc.chart.draw(google.visualization.arrayToDataTable(f),rc.CHART_OPTIONS)}else rc.$chart.css("visibility","hidden"),console.error("Chart not ready")};
rc.buildDot=function(a,c){var b=a.date,f=b.toString().split(" ")[1]+" "+b.getDate()+", "+b.getFullYear();c=c||a.owed().toFloat();var d=a.owed().toString();return[{v:b,f:f},{v:c,f:d}]};rc.checkOverflow=function(){var a=rc.$column.width();rc.lastColWidth!==a&&(rc.lastColWidth=a,rc.$chart.css({width:a+"px",height:a+"px"}),rc.drawChart(),rc.largeTableWidth||(rc.largeTableWidth=rc.$table.width()),rc.viewModel.isSmall(rc.largeTableWidth>a))};
window.onload=function(){rc.$table=$("#dateTable");rc.$column=rc.$table.parent();rc.$chart=$("#chart");rc.storage=new StorageManager;rc.viewModel=new PageViewModel(rc.storage,rc.highlightPoints,rc.drawChart);ko.applyBindings(rc.viewModel);rc.viewModel.loadSavedData();rc.checkOverflow();$(window).resize(rc.checkOverflow);rc.completeLoadDate()};
