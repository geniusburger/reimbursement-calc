...
window.onload=function(){rc.storage=new StorageManager;$("#inputButton").on("click",null,rc.getInput);$("#testButton").on("click",null,rc.loadTestData);$("#clearButton").on("click",null,dateRowUtil.deleteAll);$("#inputAmount").keypress(rc.enterCatch);$("#inputDate").keypress(rc.enterCatch);rowUtil.table=document.getElementById("tableBody");sizeRowUtil.add();rc.$table=$("#dateTable");rc.$column=rc.$table.parent();rc.$chart=$("#chart");rc.checkOverflow();todayRowUtil.add();rc.populateTimeAmounts("Years",
2);var a=$("#timeUnit");if(window.isRunningLocally())rc.loadTestData();else{rc.storage.displayCookieWarning&&$("#cookieAlert").removeClass("hidden");var b=rc.storage.getTimeAmount(),c=rc.storage.getTimeUnit();null!==b&&null!==c&&""!==b&&""!==c&&(a.val(c),rc.populateTimeAmounts(c,b));rc.storage.getDates().forEach(function(a){rc.loadDate(a)})}$("#timeAmount").on("change",null,rc.timeAmountChanged);a.on("change",null,rc.timeUnitChanged);$(window).resize(rc.checkOverflow)};
