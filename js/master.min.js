var util={removeChildren:function(a){for(;a.firstChild;)a.removeChild(a.firstChild)},getNumberWithCommas:function(a){return a.toString().replace(/\B(?=(\d{3})+(?!\d))/g,",")},getSelected:function(a){return document.getElementById(a).selectedOptions[0].value},isInvalidDate:function(a){return isNaN(a.getTime())},toArray:function(a){return Array.prototype.slice.call(a)},stripTime:function(a){if(a instanceof Date)return new Date(a.getTime()-36E5*a.getHours()-6E4*a.getMinutes()-1E3*a.getSeconds()-a.getMilliseconds())}};
function CookieManager(){this.names={DATES:"dates",TIME_AMOUNT:"timeAmount",TIME_UNIT:"timeUnit"}}CookieManager.prototype.getTimeUnit=function(){return this.getCookie(this.names.TIME_UNIT)};CookieManager.prototype.setTimeUnit=function(a){this.setCookie(this.names.TIME_UNIT,a)};CookieManager.prototype.getTimeAmount=function(){return this.getCookie(this.names.TIME_AMOUNT)};CookieManager.prototype.setTimeAmount=function(a){this.setCookie(this.names.TIME_UNIT,a)};
CookieManager.prototype.getDates=function(){var a=this.getCookie(this.names.DATES),b=[];null!==a&&""!==a&&a.split(":").forEach(function(a){var c=a.split("@");2===c.length?b.push(c):console.warn("invalid date cookie",a)});return b};CookieManager.prototype.setDates=function(a){var b="";"undefined"===typeof a&&(a=dateRowUtil.getStartRows());a.forEach(function(a,c){c&&(b+=":");b+=a.row.amountCell.toString()+"@"+a.row.dateCell.toString()});this.setCookie(this.names.DATES,b)};
CookieManager.prototype.getCookie=function(a){a+="=";for(var b=document.cookie.split(";"),d=0;d<b.length;d++){var c=b[d].trim();if(0==c.indexOf(a))return c.substring(a.length,c.length)}return null};CookieManager.prototype.setCookie=function(a,b){document.cookie=a+"="+b};
...
rc.checkOverflow=function(){var a=rc.$column.width();if(rc.lastColWidth!==a){rc.lastColWidth=a;rc.$chart.css({width:a+"px",height:a+"px"});rc.drawChart();var b=rc.$table.width()>a;rc.widthWhenChangingSmallSize?a!=rc.widthWhenChangingSmallSize&&cellUtil.setSmallSize(b)&&(rc.widthWhenChangingSmallSize=a):cellUtil.setSmallSize(b)&&(rc.widthWhenChangingSmallSize=a)}};
window.onload=function(){rc.cookies=new CookieManager;$("#inputButton").on("click",null,rc.getInput);$("#testButton").on("click",null,rc.loadTestData);$("#clearButton").on("click",null,dateRowUtil.deleteAll);$("#inputAmount").keypress(rc.enterCatch);$("#inputDate").keypress(rc.enterCatch);rowUtil.table=document.getElementById("tableBody");sizeRowUtil.add();rc.$table=$("#dateTable");rc.$column=rc.$table.parent();rc.$chart=$("#chart");rc.checkOverflow();todayRowUtil.add();rc.populateTimeAmounts("Years",
2);var a=$("#timeUnit");if(window.isRunningLocally())rc.loadTestData();else{!0!==navigator.cookieEnabled&&$("#cookieAlert").removeClass("hidden");var b=rc.cookies.getCookie("timeAmount"),c=rc.cookies.getCookie("timeUnit");null!==b&&null!==c&&""!==b&&""!==c&&(a.val(c),rc.populateTimeAmounts(c,b));rc.cookies.getDates().forEach(function(a){rc.loadDate(a)})}$("#timeAmount").on("change",null,rc.timeAmountChanged);a.on("change",null,rc.timeUnitChanged);$(window).resize(rc.checkOverflow)};
